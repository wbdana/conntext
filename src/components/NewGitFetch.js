...
class NewGitFetch extends React.Component {
  state = {
    githubUsername: '',
    repoName: '',
    filePath: '',
  ...
  fetchUserRepos = () => {
    ...
    fetch(`https://api.github.com/users/${this.state.githubUsername}/repos?sort=pushed`, options)
      ...
      .then(parsed => {
        ...
        for (i = 1; i < parseInt(parsed.last.page) + 1; i++) {
    ...
    return(
      <Container>
        <Container textAlign='center' className="gitFetch">
          <div>Know exactly what you need? <NavLink to="/gitfetchdirect" exact>Try GitFetch Direct instead!</NavLink></div>
